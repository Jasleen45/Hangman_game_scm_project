...
while run:
    ...
    al = ['a', 'b', 'c', 'd', 'e', 'f', 'g', 'h', 'i', 'j', 'k', 'l', 'm', 'n', 'o', 'p', 'q', 'r', 's', 't', 'u',
          'v', 'w', 'x', 'y', 'z']
    for let in al:
        exec('{}=PhotoImage(file="assets/{}.png")'.format(let, let))

    # hangman images
    h123 = ['h1', 'h2', 'h3', 'h4', 'h5', 'h6', 'h7']
    for hangman in h123:
        exec('{}=PhotoImage(file="assets/{}.png")'.format(hangman, hangman))
    ...
    c1 = Label(root, bg="#E7FFFF", image=h1)
    c1.place(x=300, y=-50)
    ...
    e1 = PhotoImage(file='assets/exit.png')
